...
def findsweep(objs,files):
    """Determine which sweep files correspond to given coordinates. Modified from Dr. Myers' is_in_box and decode_sweep_name functions
    ...
    objs : :class:`~numpy.ndarray`
        An array of objects. Must include at least the columns "RA" and "DEC".
    files : :class:`list`
        list of sweep files to be searched over

    Returns
    -------
    Sweep files :class:`list`
        List of the sweep files which span the coordinates given
    """
    sweepfiles = []
    for i in files:
